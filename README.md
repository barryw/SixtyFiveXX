...
**This is not a complete core.** 199 of the 65816's 256 opcodes are implemented — phase 7b's 32,
...
first opcodes with no operand at all — and the twelve transfers, whose width comes from
the destination register rather than the source.
...
in progress — `XBA` is scoped with the transfers but is not in yet, because it is the one implied
opcode on the part that takes three cycles rather than two and the 65816 emitter has no three-cycle
implied sequence — and phase 7d adds control flow, the stack and interrupts after it.
