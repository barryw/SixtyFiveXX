...
**This is not a complete core.** 77 of the 65816's 256 opcodes are implemented — phase 7b's 32,
plus phase 7c's in-progress bulk work, which has so far added `ORA`, `AND` and `EOR` in all
fifteen addressing forms each. Every other opcode throws `UndefinedOpcodeException`. Phases 7c
and 7d add the rest.
